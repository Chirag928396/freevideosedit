        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {blogPosts.map((post) => (
            <Link
              href={`/blog/${post.slug}`}
              key={post.id}
              className="group bg-white dark:bg-zinc-900/30 border border-gray-200 dark:border-zinc-800/50 rounded-xl overflow-hidden hover:border-gray-400 dark:hover:border-zinc-700 transition-all duration-300 hover:-translate-y-1 flex flex-col shadow-sm"
            >
              {/* Featured Image (background fallback to avoid broken images) */}
              <div
                className="h-48 w-full relative overflow-hidden bg-gray-200 dark:bg-zinc-900 bg-cover bg-center"
                style={{
              ...
              <div className="p-6 flex flex-col flex-grow">
                <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-zinc-500 mb-3">
                  <span className="px-2 py-1 bg-gray-200 dark:bg-zinc-800 rounded text-gray-700 dark:text-zinc-300">
                    {post.category}
                  </span>
                  <span>{post.date}</span>
                </div>
