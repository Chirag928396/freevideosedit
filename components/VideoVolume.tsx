...
export default function VideoVolume() {
  ...
  const { loaded, isProcessing, progress, adjustVolume } = useFFmpeg();
  ...
  const handleExport = async () => {
    ...
    try {
      const blob = await adjustVolume(videoFile, level);
      const url = URL.createObjectURL(blob);
      setResultUrl(url);
      appendProcessingHistory({
        tool: "Volume",
        fileName: `volume-${level}-${videoFile.name}.mp4`,
        sizeBytes: blob.size,
      });
      const a = document.createElement("a");
      a.href = url;
      a.download = `volume-${level}-${videoFile.name.replace(/\.[^.]+$/, "")}.mp4`;
      a.click();
    } catch (e) {
      console.error(e);
      alert(
        e instanceof Error
          ? e.message
          : "Failed to adjust volume (video may have no audio)",
      );
    }
  };
      ...
      <div className="rounded-2xl border border-gray-200 bg-white/80 p-6 shadow-sm backdrop-blur-sm dark:border-zinc-800/50 dark:bg-zinc-900/50 dark:shadow-2xl">
        {!videoUrl ? (
          <VideoDropZone onVideoFile={setFile}>
            <label htmlFor="video-upload-vol" className="block cursor-pointer">
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-zinc-800/50 dark:bg-zinc-900/50">
              ...
              <p className="mt-2 text-xs text-gray-500 dark:text-zinc-500">
                0% is silent; 100% is original level; 200% boosts loudness (may
                clip).
              </p>
            </div>
